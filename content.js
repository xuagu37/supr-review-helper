// content.js (SUPR scraper: Proposal fields + Resources requested + Past Resource Usage (PI, Berzelius-* only))
...
<div id="supr-status" style="padding:0 10px 10px 10px; font-size:12px; color:#555; display:flex; align-items:center; gap:8px;">
  <span id="supr-spinner" style="display:none;">⏳</span>
  <span id="supr-status-text"></span>
</div>
...
const btn = document.getElementById("supr-generate");
const spinner = document.getElementById("supr-spinner");
const statusText = document.getElementById("supr-status-text");

const prompt = output.value;
if (!prompt) return;

// UI: disable + show spinner + timer
btn.disabled = true;
const oldLabel = btn.innerText;
btn.innerText = "Generating...";
if (spinner) spinner.style.display = "inline";
if (statusText) statusText.textContent = "Generating... 0s";

const t0 = Date.now();
const timer = setInterval(() => {
  const secs = Math.floor((Date.now() - t0) / 1000);
  if (statusText) statusText.textContent = `Generating... ${secs}s`;
}, 500);

try {
  output.value += "\n\n====================\nGenerated Review (Possibility + Efficiency + Track Record + Suggested Allocation)\n====================\n";
  ...
  clearInterval(timer);
  const secs = Math.floor((Date.now() - t0) / 1000);
  if (statusText) statusText.textContent = `Done (${secs}s)`;
...
// --- Resources requested on proposal page ------------------------------------------
...
function scrapeProposal() {
  ...
  const dataManagementPlan = extractLongFromShowValue("Data Management Plan");
